...
/**
 * Loads chat history from chat-service and converts it for provider-specific
 * replay. Tool audit rows are filtered out; USER/ASSISTANT text stays.
 */
@Service
public class HistoryReplayer {
    ...
    public List<MessageParam> toParams(List<MessageDto> rows) {
        ...
        try {
            ...
            for (MessageDto m : rows) {
                ...
                switch (m.role()) {
                    case USER -> out.add(MessageParam.builder()
                            .role(MessageParam.Role.USER)
                            .content(content)
                            .build());
                    case ASSISTANT -> out.add(MessageParam.builder()
                            .role(MessageParam.Role.ASSISTANT)
                            .content(content)
                            .build());
                    case TOOL_CALL, TOOL_RESULT -> { /* already filtered */ }
                }
            }
            return out;
        } catch (Exception e) {
    ...
    public List<MessageDto> loadRows(UUID sessionId, UUID userId, String currentMessage) {
