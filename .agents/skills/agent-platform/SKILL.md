---
name: agent-platform
description: Self-hosted mobile agent platform — Java 21 + Spring Boot 3.5 + Spring Cloud + Spring AI + React + Kotlin Compose Android. LLM drives tools running on the user's bound Android phone via WebSocket. Use this skill when the user asks about architecture, device tools, prompt/persona/skills/memory, Codex or Anthropic provider routing, semantic photo search, session UI, Android builds/APK install, GitNexus workflow, GHCR image publishing, or megumin docker-compose deployment.
---
...
- After installing the Android APK via ADB, always confirm/enable the Agent Platform accessibility service (`com.agentplatform.android/com.agentplatform.android.ui.accessibility.UiAccessibilityService`) before handing back. Preserve existing enabled accessibility services when updating `enabled_accessibility_services`, set `accessibility_enabled=1`, and verify with `settings get secure enabled_accessibility_services` plus `dumpsys accessibility`.
...
- APK install preference: first check ADB. If a device is connected, install with ADB and enable accessibility; do not publish or provide the APK download URL. Only when ADB is unavailable and the user needs remote install should you publish `build/apk-publish/agent-platform-debug.apk` and provide `https://agentdl.rainaki.top/agent-platform-debug.apk`.
